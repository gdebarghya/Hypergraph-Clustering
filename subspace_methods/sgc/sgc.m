function [sampleLabels,averageL2Error] = sgc(X,d,K,OPTIONS)

%   SGC: Code by Debarghya Ghoshdastidar
%
...
%
%   [ ... ] = sgc(..., OPTIONS) allows you to specify optional parameters 
...
trackSubspace = struct('flag',false,'U',zeros(N,K));
...
%for iii = 1    
    ...
    if OPTIONS.findOptimalSigma == 0 %% use a single sigma
        ...
        [sampleLabels1,trackSubspace] = sgc_processing_affinities(A,K,trackSubspace,OPTIONS);
        ...
        switch OPTIONS.search_by
            ...
            case 'index'
                ...
                while p <= min(OPTIONS.n-1,p_max) && sigma >= ABSOLUTE_MINIMUM
                    ...
                    [sampleLabels2,trackSubspace] = sgc_processing_affinities(A,K,trackSubspace,OPTIONS);
