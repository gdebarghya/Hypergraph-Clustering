function [sampleLabels,averageL2Error,optimalSigma] = tetris(X,d,K,OPTIONS)

%   Tensor Trace maximization with Iterative Sampling (TeTrIS)
%
%   [sampleLabels, averageL2Error,optimalSigma] = tetris(X,d,K) 
...
%   TeTrIS returns an N-by-1 vector sampleLabels containing the cluster 
%   label of each point and the averaged L2 error of the K detected 
%   clusters. Those with zero labels are detected outliers. 
%   optimalSigma is value of hyperparameter sigma, also determined through 
%   the iterations.
%
%   [ ... ] = tetris(..., OPTIONS) allows you to specify optional parameters 
...
while averageL2Error1 < averageL2Error * 0.99 || q>2
    ...
    if OPTIONS.findOptimalSigma == 0 %% use a single sigma
        ...
        sampleLabels1 = tetris_processing_affinities(A,sampledColumns,K,OPTIONS);
        ...
        switch OPTIONS.search_by
            ...
            case 'index'
                ...
                while p <= min(OPTIONS.n-1,p_max) && sigma >= ABSOLUTE_MINIMUM
                    ...
                    sampleLabels2 = tetris_processing_affinities(A,sampledColumns,K,OPTIONS);
